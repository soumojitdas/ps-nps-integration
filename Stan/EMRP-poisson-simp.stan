data {
  ...
  // Cell mapping for overlap observations
...
generated quantities {
  // EMRP: Impute Y2 for NPS sample (not Y1 for PS)
  array[N_nps] int Y2_nps_imputed;
  vector[N_nps] prob_Y2_given_Y1;
  
  // Calculate P(Y2=1|Y1) for each NPS individual
  for (i in 1:N_nps) {
    if (Y1_nps[i] == 1) {
      // P(Y2=1|Y1=1) = sensitivity
      prob_Y2_given_Y1[i] = sens_prob;
    } else {
      // P(Y2=1|Y1=0) = 1 - specificity
      prob_Y2_given_Y1[i] = 1 - spec_prob;
    }
