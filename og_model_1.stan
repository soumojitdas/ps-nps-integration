// OG Model 1: Y1 = X*beta + alpha*Y2
// Simplest model with only global Y2 effect
...
transformed parameters {
  ...
  // Linear predictors for Y1 base (demographics only)
...
model {
  ...
  for (i in 1:N_nps) {
    ...
    // P(Y1=1|Y2=1) - only global alpha effect
  ...
  // Y1 given Y2 with only global alpha effect
  for (i in 1:N_overlap) {
...
generated quantities {
  ...
  for (c in 1:C) {
    ...
    real p_y1_given_y2_1 = inv_logit(X_cell[c] * beta + alpha);
  ...
  // Store total Y2 effect for diagnostic purposes
  real total_y2_effect = alpha;  // Constant across all areas
}
